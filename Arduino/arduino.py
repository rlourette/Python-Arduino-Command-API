...
def build_cmd_str(cmd, args=None):
    ...
    """
    if args:
        args = r'%'.join(map(str, args))
    else:
        args = r''
    return bytes(f"%{cmd}{args}$!", 'utf-8')
...
def find_port(baud, timeout):
    ...
    if platform.system() == 'Windows':
        ...
        ports = glob.glob(b"/dev/ttyUSB*") + glob.glob(b"/dev/ttyACM*")
    for p in ports:
...
class Arduino(object):
    def __init__(self, baud=9600, port=None, timeout=2, sr=None):
        ...
        if not sr:
            if not port:
                ...
                if not sr:
                    raise ValueError(b"Could not find port.")
            else:
    ...
    def digitalWrite(self, pin, val):
        ...
        # try:
        # print("digitalWrite: " + str(cmd_str))
        self.sr.write(cmd_str)
        self.sr.flush()
        # rd = self.sr.readall()
        # print("digitalWrite rd: " + str(rd))
        # except:
        #     pass
    ...
    def Melody(self, pin, melody, durations):
        ...
        when playing it multiple times.
            board.Melody(9,["C4","G3","G3","A3","G3",0,"B3","C4"],
                                                [4,8,8,4,4,4,4,4])
        The hardware buffer in the Arduino is 64 bytes.  Batching the length
        of tones to 8 will prevent overruns at higher baudrates
        """
        assert(len(melody) == len(durations))
        maxNotesAtATime = 8
        retval = 0
        NOTES = dict(
            ...
            AS7=3729, B7=3951, C8=4186, CS8=4435, D8=4699, DS8=4978, REST=0)
        if (isinstance(melody, list)) and (isinstance(durations, list)):
            length = len(melody)
            if length == len(durations):
                for offset in range(0, length, maxNotesAtATime):
                    notes = [NOTES.get(melody[idx]) for idx in range(offset, min(offset+maxNotesAtATime, length))]
                    cmd_args = [len(notes), pin]
                    # interleave the two lists
                    cmd_args.extend([val for pair in zip(notes, durations[offset:offset+maxNotesAtATime]) for val in pair])
                    ...
                    # %tO8%5%262%4%196%8%196%8%220%4%196%4%None%4%247%4%262%4$!
                    cmd_str = build_cmd_str("to", cmd_args)
                    print(len(cmd_str))
                    assert(len(cmd_str) <= 64)
                    if True: # try:
                        self.sr.write(cmd_str)
                        self.sr.flush()
                        rd = self.sr.readline().strip()
                        print("NewMelody: "+ str(rd))
                        numplayed = int(rd)
                        retval += numplayed
                        if len(notes) != int(rd):  # try:
                            return retval
                    # except:
                    #     pass
            else:
    ...
    def shiftOut(self, dataPin, clockPin, pinOrder, value):
        ...
        Input:
            ...
            pinOrder (String): either 'M' or 'L' for 'MSBFIRST' and 'LSBFIRST' respectively
    ...
    def scanI2C(self):
        ...
        retval = []
        while True:
            addr = int(self.sr.readline().strip())
            if addr:
                retval.append(addr)
            else:
                break
...
class SoftwareSerial(object):
    ...
    def begin(self, p1, p2, baud):
        ...
        # except:
        #     pass
        response = self.sr.readline().strip()
        if response == "ss OK":
            self.connected = True
            return True
        else:
